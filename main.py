...
if __name__ == "__main__":
    ...
    print(f"🧮 IPCA Spreads calculados (depois do filtro): {len(corp_base_ipca)}")
    ...
    fig_di_surface = plot_yield_curve_surface(df_vis, source_text="Source: DI B3 – cálculos próprios")
    ...
    # ---------------------------- IPCA BLOCO ----------------------------
    ipca_curve = pd.read_excel(CONFIG["WLA_CURVE_PATH"], sheet_name="only_values")
    ...
    print(f"🧮 IPCA Spreads calculados (antes do filtro): {len(corp_bonds_ipca)}")
    corp_bonds_ipca = anomaly_filtering_results(corp_bonds_ipca)
    print(f"🧼 IPCA Spreads restantes após filtros de anomalias: {len(corp_bonds_ipca)}")
    ...
    pd.DataFrame(skipped_ipca, columns=["Bond ID", "Obs Date", "Reason"]).to_csv("data/skipped_ipca_yields.csv", index=False)
